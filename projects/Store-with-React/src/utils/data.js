...
//? this is done since many ids of objects were same in the above array of objects, so below one makes them unique
/*
! data.flatMap(item => item.products)
* Converts nested structure into one flat array of products.
* Output looks like: `[p1, p2, p3, ...]

! .map(p => [p.id, p])
* Turns each product into a 2-element pair: [id, product]
* Example: [[162, p1], [162, p2], [163, p3]]

! new Map(pairs)
* A Map can’t have duplicate keys (id).
* If the same id appears again, the later one overwrites the earlier one.
* So only one product per id remains in the Map.

! .values()
* Gets back only the unique product objects (values of the Map).

! Array.from(...)
* Converts the Map.values() iterable into a normal array.
* So unique ends up as: “only one object per id”, no repeated ids.

*/
const unique = Array.from(
  new Map(
    data.flatMap(item => item.products).map(p => [p.id, p])
  ).values()
);
